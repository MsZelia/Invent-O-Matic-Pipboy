package Mobile.ScrollList
{
   ...
   public class MobileScrollList extends MovieClip
   {
      ...
      protected function setPosition() : void
      {
         ...
         var scrollDim:Number = this._direction == HORIZONTAL ? this._scrollList.width : this._scrollList.height;
         var boundsDim:Number = this._direction == HORIZONTAL ? this._bounds.width : this._bounds.height;
         var scrollPos:Number = this._direction == HORIZONTAL ? this._scrollList.x : this._scrollList.y;
         if(isNaN(this.position))
         {
            if(this.selectedIndex == -1)
            {
               ...
               {
                  this._scrollList.x = this.endListAlign ? boundsDim - scrollDim : 0;
               }
               else
               {
                  this._scrollList.y = this.endListAlign ? boundsDim - scrollDim : 0;
               }
               this.setDataOnVisibleRenderers();
               return;
            }
            selectedRendererPos = this._direction == HORIZONTAL ? this.selectedRenderer.x : this.selectedRenderer.y;
            if(this.canScroll)
            {
            ...
            {
               this._position = this.endListAlign ? boundsDim - scrollDim : 0;
            }
         }
      ...
      protected function itemSelectHandler(e:EventWithParams) : void
      {
         ...
         if(this.clickable)
         {
            ...
            rendererId = int(renderer.data.id);
      ...
      protected function enterFrameHandler(e:Event) : void
      {
         ...
         if(this._bounds != null && this.canScroll)
         {
            ...
            scrollDim = this._direction == HORIZONTAL ? this._scrollList.width : this._scrollList.height;
            maxDim = this._direction == HORIZONTAL ? this._bounds.width : this._bounds.height;
            scrollPos = this._direction == HORIZONTAL ? this._scrollList.x : this._scrollList.y;
            if(!this._mouseDown)
            {
               bouncing = 0;
            ...
            if(!isNaN(e.stageX) && !isNaN(e.stageY))
            {
               ...
               if(this._direction == HORIZONTAL)
               {
                  ...
                  if(this._scrollList.x >= this._bounds.x || this._scrollList.x <= this._bounds.x - (this._scrollList.width - this._bounds.width))
                  {
                     if(this.elasticity)
                     {
                        this._scrollList.x += nextScrollListPos * this.RESISTANCE_OUT_BOUNDS;
                     }
                     ...
                     {
                        this._scrollList.x += nextScrollListPos;
                     }
                  }
                  else
                  {
                  ...
                  this._velocity += (point.x - this._prevMouseDownPoint.x) * this.VELOCITY_MOVE_FACTOR;
               }
               else
               {
                  ...
                  if(this._scrollList.y >= this._bounds.y || this._scrollList.y <= this._bounds.y - (this._scrollList.height - this._bounds.height))
                  {
                     if(this.elasticity)
                     {
                        this._scrollList.y += nextScrollListPos * this.RESISTANCE_OUT_BOUNDS;
                     }
                     ...
                     {
                        this._scrollList.y += nextScrollListPos;
                     }
                  }
                  else
                  {
                  ...
                  this._velocity += (point.y - this._prevMouseDownPoint.y) * this.VELOCITY_MOVE_FACTOR;
               }
